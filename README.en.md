...
3. **Convert** (`convert.ts`, pure function):
   ...
   - a model entry's `input` declaration (e.g. `["text", "image"]`) carries through as the model's input modalities — this is how dsh decides image capability; when undeclared it falls back to the same-named pi-ai catalog model, and to text-only when the catalog has none;
...
| Images | supported (via dsh-attachment) | supported (also via dsh-attachment, same policy as official) |
